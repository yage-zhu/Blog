...
AJAX 全称为Asynchronous JavaScript And [XML](https://so.csdn.net/so/search?q=XML&spm=1002101.01.70)，就是异步的JS 和XML通过AJAX 可以在浏览器中向服务器发送异步请求，最大的优势：**无刷新获取数据**
...
![process](../images/240428-AYTtylbxQB.png)
...
![network](../images/240428-yPUUlEEZBT.png)
...
```js
      ...
      // 3. 事件绑定 处理服务端返回的结果
	  // loadend 事件总是在一个资源的加载进度停止之后被触发，无论成功还是失败
...
// 方法二
// 自动拼接对象 'name=yage&age=18'
const queryString = new URLSearchParams({name:'yage',age:18}).toString()
xhr.open('GET', `http://127.0.0.1:8000/server?${queryString}`)
```

![image-20220624212741956](../images/240428-IHXcnVhOJX.png)

![](../images/240428-YXDbmJJlXp.png)
...
![status](../images/240428-f10xnJ7oji.png)
