...
// 倒计时
const now = +new Date()
const last = +new Date('2023-12-23 17:30:00')
const count = (last - now) / 1000
// 转化为时分秒
let d = parseInt(count / 60 / 60 / 24)
let h = parseInt(count / 60 / 60 % 24)
h = h < 10 ? '0' + h : h
let m = parseInt(count / 60 % 24)
m = m < 10 ? '0' + h : h
...
`DocumentFragment`，文档片段接口，表示一个没有父对象的最小文档对象。它不是真实 DOM 树的一部分，它的变化不会触发 DOM 树的[重新渲染](https://developer.mozilla.org/zh-CN/docs/Glossary/Reflow)，且不会对性能产生影响。
